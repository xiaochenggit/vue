<template>
	<div class="ratingselect">
		<div class="rating-type">
			...
			<span @click="select(1,$event)" class="block negative" :class="{'active':selectType===1}">{{desc.negative}}<span class="count"></span></span>
		</div>
		...
		positives: function () {
			...
			this.ratings.forEach(function(rating) {
				if (rating.rateType === POSITIVE) {
					rating.push(positiveslength);
				}
			});
			return positiveslength.length;
		},
        netatives: function () {
			let netativeslength = [];
			this.ratings.forEach(function(rating) {
				if (rating.rateType === NEGATIVE) {
					rating.push(netativeslength);
				}
			});
			return netativeslength.length;
		}
	},
	methods: {
